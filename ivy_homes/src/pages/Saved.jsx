...
import { formatPrice, formatArea, bhkLabel, titleCase, formatRelativeDate } from '../utils/format';

export default function Saved() {
  const { saved, unsave } = useSavedListings();

  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-2xl text-(--color-ink)">Saved listings</h1>
        <p className="mt-1 text-sm text-(--color-ink-soft)">
          {saved.length > 0
            ? `${saved.length} listing${saved.length === 1 ? '' : 's'} saved to this account`
            : 'Kept locally on this device, tied to your account'}
        </p>
      </div>
      ...
      {saved.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {saved.map((listing) => (
            ...
            >
              <Link to={`/listings/${encodeURIComponent(listing.listing_id)}`} className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap gap-x-3 font-mono text-sm">
                  <span className="font-semibold text-(--color-brick) tabular">{formatPrice(listing.price)}</span>
                  <span className="text-(--color-ink-faint) tabular">{formatArea(listing.carpet_area)}</span>
                  <span className="text-(--color-ink-faint)">saved {formatRelativeDate(listing.saved_at)}</span>
                </div>
              </Link>
              <button
                onClick={() => unsave(listing.listing_id)}
